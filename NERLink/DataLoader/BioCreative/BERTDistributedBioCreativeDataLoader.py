...
class BERTDistributedBioCreativeDataLoader( BERTBioCreativeDataLoader ):
    ...
    def Encode_CL_Model_Data( self, data_list = [], use_csr_format = False, pad_input = True, pad_output = True, keep_in_memory = True,
        ...
        # Check(s)
        if len( data_list ) == 0:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Warning: No Data Specified By User / Using Data Stored In Memory" )
            data_list = self.data_list

        if len( data_list ) == 0:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Not Data To Vectorize / 'data_list' Is Empty", force_print = True )
            return None, None

        cl_pad_output = False

        if label_per_sub_word and pad_output:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Warning: Large Memory Consumption When 'label_per_sub_word = True' and 'pad_output = True' / Setting 'pad_output = False'" )
            pad_output = False

        if not pad_output:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Warning: CL Output Requires 'pad_output = True' / Forcing 'cl_pad_output = True'" )
            cl_pad_output = True
        ...
        if self.max_sequence_length > self.max_sequence_limit:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Warning: Max Sequence Length > " + str( self.max_sequence_limit ) + " / Enforcing BERT Max Sequence Length == "  + str( self.max_sequence_limit ) )
            self.max_sequence_length = self.max_sequence_limit

        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Encoding Concept Instances" )
        ...
        output_instance_index, sequence_length            = 0, 0
        cui_less_id                                       = self.Get_Concept_ID( self.Get_CUI_LESS_Token() )
        padding_id                                        = self.Get_Concept_ID( self.Get_Padding_Token() )
        default_cui_less_id                               = cui_less_id if use_cui_less_labels else padding_id
        ...
        for passage_idx, passage in enumerate( data_list ):
            ...
            if passage_sequence == "" or len( passage_sequence ) == 0:
                self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Text Sequence Is Empty String" )
                continue

            if len( annotations ) == 0 or len( annotation_concept_ids ) == 0:
                self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Passage Contains No NER or CL Labels" )
                self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -        Passage: " + str( passage.Get_Passage() ) )
                continue

            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Encoding Inputs" )
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -                   Text Sequence         : " + str( passage_sequence      ) )
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -                   Annotations           : " + str( annotations           ) )
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -                   Annotation Indices    : " + str( annotation_indices    ) )
            ...
            for idx, ( annotation, annotation_idx, is_composite_mention, is_individual_mention ) in enumerate( zip( annotations, annotation_indices,
                ...
                for indices in annotation_idx.split( "<:>" ):
                    ...
                    if number_of_indices == 1 and annotation != extracted_token:
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Extracted Token != True Token", force_print = True )
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -        True Token: " + str( annotation ), force_print = True )
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -        Extracted Token: " + str( extracted_token ), force_print = True )
                        continue
                    elif number_of_indices > 1 and extracted_token not in annotation:
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Extracted Token Not In True Token", force_print = True )
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -        True Token: " + str( annotation ), force_print = True )
                        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() -        Extracted Token: " + str( extracted_token ), force_print = True )
                        continue
            ...
            subword_tokens, processed_annotation_ids, term_mask, subword_concept_label_ids = [], [], [], []
            ...
            for ( index, token ), token_annotation_id in zip( enumerate( passage_sequence.split() ), text_sequence_annotation_indices ):
                ...
                if len( annotation_ids ) > 0:
                    ...
                    subword_concept_label_ids.extend( [[default_cui_less_id]] * len( token_subwords ) )
                    ...
                    for annotation_id in annotation_ids:
                        ...
                        if ignore_output_errors and -1 in concept_ids:
                            ...
                            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Warning: Concept ID Not Found While Encoding Output" )
                            self.Print_Log( "                                                             - Desired Concept(s)    : " + str( desired_concepts ) )
                            self.Print_Log( "                                                             - Obtained Concept iD(s): " + str( concept_ids      ) )
                            concept_ids = [default_cui_less_id]
                        elif not ignore_output_errors and -1 in concept_ids:
                            desired_concepts = annotation_concept_ids[annotation_id_to_idx[annotation_id]].split( concept_delimiter )
                            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error Encoding Concept ID Output / Concept ID Not Found In Desired Concept(s)" )
                            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Desired Concept(s)    : " + str( desired_concepts ) )
                            self.Print_Log( "                                                             - Obtained Concept iD(s): " + str( concept_ids      ) )
                            encoding_error = True
                            break

                        temp_word, start_tagged = "", False if annotation_id not in processed_annotation_ids else True
                        current_idx = len( subword_concept_label_ids ) - ( len( token_subwords ) )
                    ...
                    term_mask.extend( [0] * len( token_subwords ) )
                    subword_concept_label_ids.extend( [[cui_less_id]] * len( token_subwords ) )

            # Check For Error Occurring During Output Encoding And Skip Passage Instance If Detected
            if encoding_error:
                self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error Encoding Passage: " + str( passage_sequence ), force_print = True )
                continue
            ...
            encoded_cl_labels  = [[default_cui_less_id]] + subword_concept_label_ids + [[default_cui_less_id]]
            ...
            if ( pad_input or pad_output or cl_pad_output ) and self.max_sequence_length - len( token_ids ) > 0:
                ...
                encoded_cl_labels.extend( [[padding_id]] * padding_length )
            ...
            if split_by_max_seq_length and sequence_length > self.Get_Max_Sequence_Length():
                ...
                while split_start_idx < sequence_length:
                    ...
                    if encoded_cl_labels[split_end_idx] not in [[padding_id], [cui_less_id]]:
                        ...
                        while temp_split_end_idx > split_start_idx and encoded_cl_labels[temp_split_end_idx] not in [[padding_id], [cui_less_id]]:
                    ...
                    split_instance_term_mask = term_mask[split_start_idx:split_end_idx]
                    ...
                    if skip_remaining_splits or all( val == 0 for val in split_instance_term_mask ):
                    ...
                    if split_token_ids[0]  != self.sub_word_cls_token_id:
                        ...
                        split_instance_term_mask.insert( 0, 0 )
                        split_encoded_cl_output.insert( 0, [default_cui_less_id] )
                    ...
                    if split_token_ids[-1] != self.sub_word_sep_token_id:
                        ...
                        split_instance_term_mask.append( 0 )
                        split_encoded_cl_output.append( [default_cui_less_id] )
                    ...
                    if self.Get_Max_Sequence_Length() - len( split_token_ids ) > 0:
                        ...
                        split_instance_term_mask.extend( [self.sub_word_pad_token_id] * padding_length )
                        split_encoded_cl_output.append( [padding_id] * padding_length )
                    ...
                    encoded_entry_term_masks.append( split_instance_term_mask )
                    ...
                    if use_csr_format:
                        ...
                        for i, values in enumerate( split_encoded_cl_output ):
                            ...
                            if label_per_sub_word and ( pad_output or cl_pad_output ):
                                for value in values:
                                    output_row.append( output_instance_index )
                                    output_col.append( i )
            ...
            # No Splits Required
                ...
                encoded_entry_term_masks.append( term_mask )
        ...
        if use_csr_format:
            ...
            if label_per_sub_word and ( pad_output or cl_pad_output ):
                encoded_concept_outputs = COO( [ output_row, output_col, output_depth ], output_data, shape = ( output_instance_index, number_of_output_rows, number_of_labels ), fill_value = 0 )
            else:
                encoded_concept_outputs = COO( [ output_row, output_col ], output_data, shape = ( output_instance_index, number_of_output_rows ), fill_value = 0 )
        else:
        ...
        if number_of_input_instances == 0:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error Occurred While Encoding Concept Inputs", force_print = True )
            return None, None
        elif number_of_output_instances == 0:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error Occurred While Encoding Concept Outputs", force_print = True )
            return None, None
        elif number_of_input_instances != number_of_output_instances:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Error: Number Of Input And Output Instances Not Equal", force_print = True )
            return None, None

        if keep_in_memory:
            self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Storing Encoded Data In Memory" )
        ...
        self.Print_Log( "BERTDistributedBioCreativeDataLoader::Encode_CL_Model_Data() - Complete" )
...
if __name__ == '__main__':
    ...
    print( "     from NERLink.DataLoader.BioCreative import BioDistributedCreativeDataLoader\n" )
    print( "     data_loader = BioDistributedCreativeDataLoader( print_debug_log = True )" )
