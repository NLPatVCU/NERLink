...
#    Revised: 11/12/2022                                                                   #
#                                                                                          #
#    Generates A Bi-LSTM Neural Network Used For BioCreative 2021 - Task 2.                #
#                                                                                          #
#    Authors:                                                                              #
#    --------                                                                              #
...
if re.search( r"^2.\d+", tf.__version__ ):
    ...
    from tensorflow.keras.layers import Activation, BatchNormalization, Bidirectional, Concatenate, Dense, Dot, Dropout, Embedding, Input, Lambda, Layer, LSTM, TimeDistributed
else:
    import keras.backend as K
    from keras        import optimizers
    from keras.models import Model
    from keras.layers import Activation, BatchNormalization, Bidirectional, Concatenate, Dense, Dot, Dropout, Embedding, Input, Lambda, Layer, LSTM, TimeDistributed
    # from keras_contrib.layers import CRF  # Need 'keras_contrib' Package To Use CRF
...
class ModelDataGenerator( DataGenerator ):
    ...
    def __getitem__( self, index ):
        ...
        X_batch = self.X[batch_indices,:].todense()
        ...
        return X_batch, Y_batch, self.sample_weights, sample_weights
...
class AttentionAlt( Layer ):
    def __init__( self, units = 128 ):
        self.units = units
        super( AttentionAlt, self ).__init__()
...
class BiLSTMModel( BaseModel ):
    def __init__( self, print_debug_log = False, write_log_to_file = False, optimizer = 'adam', activation_function = 'relu',
                  loss_function = "categorical_crossentropy", number_of_embedding_dimensions = 200, bilstm_dimension_size = 64,
        ...
        self.version       = 0.17
        self.network_model = "ner_bilstm"   # Force Setting Model To 'Bi-LSTM' Model.
    ...
    def Fit( self, inputs = None, outputs = None, val_inputs = None, val_outputs = None, epochs = None, batch_size = None,
        ...
        # Check(s)
        if inputs is None or outputs is None or len( outputs ) == 0   or \
            isinstance( inputs, csr_matrix ) and inputs.shape[0] == 0 or \
            isinstance( inputs, list )       and len( inputs ) == 0:
            self.Print_Log( "BiLSTMModel::Fit() - Error: Expected Input/Output Data Length Mismatch", force_print = True )
            return False

        # Class Weight Check
        if class_weights:
            self.Print_Log( "BiLSTMModel::Fit() - Warning: Class Weights Not Supported / Setting 'class_weights = None'", force_print = True )
        ...
        with tf.device( self.device_name ):
            ...
            if self.Get_Is_Using_TF1() and self.use_csr_format:
                ...
                if validation_data:
                    validation_instances = validation_data.shape[0]
    ...
    def Predict( self, inputs, verbose = 0 ):
        # Check(s)
        if inputs is None or isinstance( inputs, csr_matrix ) and inputs.shape[0] == 0 or \
            isinstance( inputs, list ) and len( inputs ) == 0:
            self.Print_Log( "BiLSTMModel::Predict() - Error: Expected Input/Output Data Length Mismatch", force_print = True )
            return []
    ...
    def Evaluate( self, inputs, outputs, verbose ):
        ...
        if inputs is None or outputs is None or len( outputs ) == 0   or \
            isinstance( inputs, csr_matrix ) and inputs.shape[0] == 0 or \
            isinstance( inputs, list )       and len( inputs ) == 0:
            self.Print_Log( "BiLSTMModel::Evaluate() - Error: Expected Input/Output Data Length Mismatch", force_print = True )
            return -1, -1, -1, -1, -1
    ...
    """
        ...
        Inputs:
            ...
            bilstm_dimension_size          : (Integer)
            bilstm_merge_mode              : (String)
            embeddings                     : Numpy 2-Dimensional Array

        Outputs:
            None
    """
    def Build_Model( self, max_sequence_length, number_of_unique_terms, embedding_dimension_size,
                     number_of_inputs, number_of_outputs, bilstm_dimension_size = 64, bilstm_merge_mode = "concat",
                     use_attention = False, use_crf = False, embeddings = [] ):
        # Update 'BaseModel' Class Variables
        ...
        if self.use_batch_normalization:
            ...
            if( len( embeddings ) > 0 ):
                embedding_layer     = Embedding( input_dim = number_of_unique_terms, output_dim = embedding_dimension_size, input_length = max_sequence_length, name = 'Embedding_Layer', weights = [embeddings], trainable = self.trainable_weights )( input_layer )
            else:
                embedding_layer     = Embedding( input_dim = number_of_unique_terms, output_dim = embedding_dimension_size, input_length = max_sequence_length, name = 'Embedding_Layer', trainable = self.trainable_weights )( input_layer )
            ...
            bilstm_layer            = Bidirectional( LSTM( self.bilstm_dimension_size, return_sequences = True, dropout = self.dropout ), merge_mode = self.bilstm_merge_mode, name = 'BiLSTM_Layer_2' )( batch_norm_layer )
            dense_layer             = Dense( units = 256, activation = 'relu', name = 'Dense_Layer_1', use_bias = True )( bilstm_layer )
            ...
            # if use_crf:
            #     crf                 = CRF( units = number_of_outputs )
        ...
        # Traditional Model Without Batch Normalization Using Functional API
            ...
            # Determine LSTM Unit Length
            bilstm_embedding_size   = self.bilstm_dimension_size * 2 if self.bilstm_merge_mode == "concat" else self.bilstm_dimension_size

            bilstm_layer            = Bidirectional( LSTM( units = self.bilstm_dimension_size, return_sequences = True, dropout = self.dropout, recurrent_dropout = self.dropout ), merge_mode = self.bilstm_merge_mode, name = 'BiLSTM_Layer_1' )( embedding_layer )
            bilstm_layer            = LSTM( units = bilstm_embedding_size, return_sequences = True, dropout = self.dropout, recurrent_dropout = self.dropout, name = 'LSTM_Layer_1' )( bilstm_layer )

            if use_attention:
                self.Print_Log( "BiLSTMModel::Build_Model() - Warning: Bi-LSTM w/ Attention Has Not Been Fully Tested", force_print = True )
                continue_query = input( "Continue? (Y/N)\n" )
                if re.search( r"[Nn]", continue_query ): exit()
                bilstm_layer = Attention( return_sequences = True )( bilstm_layer )

            output_layer            = TimeDistributed( Dense( units = number_of_outputs, activation = self.activation_function, name = 'NER_Output_Layer' ) )( bilstm_layer )
        ...
        if use_crf:
            ...
            # Update Loss Function To CRF Loss Function
            self.loss_function = crf.loss_function

            self.model.compile( loss = self.loss_function, optimizer = adam_opt, metrics = [ crf.accuracy, 'accuracy' ] )
        elif self.optimizer == "adam":
            adam_opt = optimizers.Adam( learning_rate = self.learning_rate )
            self.model.compile( loss = self.loss_function, optimizer = adam_opt,
                                sample_weight_mode = "temporal", metrics = [ 'accuracy', super().Precision, super().Recall, super().F1_Score ] )
    ...
    #         None
    # """
    # def Load_Model( self, model_path, load_new_model ):
    #     try:
    #         if load_new_model:
    #             self.Print_Log( "BiLSTMModel::Load_Model() - Loading Pretrained Model" )
    ...
    #                                                                                              'CRF'      : CRF } )
    #             else:
    #                 self.Print_Log( "BiLSTMModel::Load_Model() - Error: Model Does Not Exist", force_print = True )
    #                 return False
    #         else:
    #             self.Print_Log( "BiLSTMModel::Load_Model() - Loading Model Configuration / Untrained Model / New Weights Per Layer" )
    ...
    #                 in_file.close()
    #             else:
    #                 self.Print_Log( "BiLSTMModel::Load_Model() - Error: Model Configuration JSON Files Does Not Exist", force_print = True )
    #                 return False
    #     except Exception as e:
    #         self.Print_Log( "BiLSTMModel::Load_Model() - Error: Unable To Load New Model Or Load Model From Configuration File" )
    ...
    #     else:
    #         self.Print_Log( "BiLSTMModel::Load_Model() - Error: Model Settings File Does Not Exist", force_print = True )
