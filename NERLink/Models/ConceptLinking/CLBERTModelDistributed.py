...
class ModelDataGenerator( DataGenerator ):
    ...
    def __getitem__( self, index ):
        ...
        Y_output          = self.Y[batch_indices,:].todense()
        ...
        return [X_token_input_ids, X_attention_mask, X_token_type_ids], Y_output, sample_weights
...
class CLBERTModelDistributed( BaseModel ):
    def __init__( self, print_debug_log = False, write_log_to_file = False, margin = 30.0, optimizer = 'adam', activation_function = 'softmax',
        ...
        self.version        = 0.06
        self.network_model  = "concept_linking_bert_distributed"
        ...
        if self.Get_Is_Using_TF2() == False:
            self.Print_Log( "CLBERTModelDistributed::__init__() - Error: BERT Model Only Supports Tensoflow >= 2.x", force_print = True )
            exit()
    ...
    def Batch_Generator( self, X, Y, batch_size, steps_per_batch, shuffle ):
        ...
        while True:
            ...
            Y_output          = Y[batch_index,:].todense()
            counter           += 1

            yield [X_token_input_ids, X_attention_mask, X_token_type_ids], Y_output
    ...
    def Fit( self, inputs = None, outputs = None, val_inputs = None, val_outputs = None, epochs = None, batch_size = None,
        ...
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModelDistributed::Fit() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return False

        if outputs is None or len( outputs ) == 0:
            self.Print_Log( "CLBERTModelDistributed::Fit() - Error: Expected Output Data Length Mismatch", force_print = True )
            return False

        # Class Weight Check
        if class_weights:
            self.Print_Log( "CLBERTModelDistributed::Fit() - Warning: Class Weights Not Supported / Setting 'class_weights = None'", force_print = True )
        ...
        #   NOTE: Entry Term Masks Are Not Used
        ...
        self.Print_Log( "CLBERTModelDistributed::Fit() - Model Training Settings" )
        ...
        self.Print_Log( "CLBERTModelDistributed::Fit() - Executing Model Training", force_print = True )
        ...
        with tf.device( self.device_name ):
            ...
            if self.Get_Is_Using_TF1() and self.use_csr_format:
                ...
                data_generator = ModelDataGenerator( X = [input_ids, attention_masks, token_type_ids], Y = outputs, batch_size = self.batch_size,
                                                     number_of_instances = self.trained_instances, shuffle = self.shuffle, sample_weights = self.Get_Sample_Weights() )
        ...
        if self.verbose == False:
            ...
            self.Print_Log( "CLBERTModelDistributed::Final Training Metric(s) At Epoch: " + str( final_epoch ), force_print = True )

            # Iterate Through Available Metrics And Print Their Formatted Values
            for metric in history.keys():
                self.Print_Log( "CLBERTModelDistributed::  - " + str( metric.capitalize() ) + ":\t{:.4f}" . format( history[metric][-1] ), force_print = True )

        self.Print_Log( "CLBERTModelDistributed::Fit() - Finished Model Training", force_print = True )
        self.Print_Log( "CLBERTModelDistributed::Fit() - Complete" )
        return True
    ...
    def Predict( self, inputs, verbose = 0 ):
        # Check(s)
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModelDistributed::Predict() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return []

        self.Print_Log( "CLBERTModelDistributed::Predict() - Predicting Using Inputs: " + str( inputs ) )
    ...
    def Evaluate( self, inputs, outputs, verbose, warmup_model = True ):
        # Check(s)
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModelDistributed::Evaluate() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return -1, -1, -1, -1, -1

        if outputs is None or len( outputs ) == 0:
            self.Print_Log( "CLBERTModelDistributed::Evaluate() - Error: Expected Output Data Length Mismatch", force_print = True )
            return -1, -1, -1, -1, -1

        self.Print_Log( "CLBERTModelDistributed::Evaluate() - Executing Model Evaluation" )
        ...
        with tf.device( self.device_name ):
            ...
            loss, accuracy = self.model.evaluate( [input_ids, attention_masks, token_type_ids], outputs,
                                                   batch_size = self.batch_size, verbose = verbose )

            self.Print_Log( "CLBERTModelDistributed::Evaluate() - Complete" )
    ...
    def Build_Model( self, max_sequence_length = 0, number_of_unique_terms = 0, embedding_dimension_size = 0,
        ...
        self.Print_Log( "CLBERTModelDistributed::Build_Model() - Model Settings" )
        ...
        if self.final_layer_type not in final_layer_type_list:
            self.Print_Log( "CLBERTModelDistributed::Build_Model() - Error: Invalid Final Layer Type", force_print = True )
        ...
        encoding_layer      = Dropout( name = "Dropout_Layer_1", rate = self.dropout )( encoding_layer )
        output_layer        = Dense( units = number_of_outputs, activation = self.activation_function, name = "Output_Layer" )( encoding_layer )

        # ========================================================================================================= #
        # NOTE: Removing These Layers In Favor Of A Single Dense Output Layer Improves Performance. (To Be Tested)  #
        #       This Code Below Is Saved For Future Experiments With CosFace, ArcFace and SphereFace Architectures. #
        # ========================================================================================================= #

        # if self.use_batch_normalization:
        #     dense_layer      = Dense( units = embedding_dimension_size, activation = 'relu', name = 'Forward_Feed_Dense_Layer_1' )( sub_word_embeddings )
        ...
        #     dense_layer      = Dense( units = number_of_outputs, activation = 'relu', name = 'Forward_Feed_Dense_Layer_2',
        ...
        #     dense_layer      = Dense( units = embedding_dimension_size, activation = 'relu', name = 'Forward_Feed_Dense_Layer_2',
        #                               kernel_initializer = 'he_normal', kernel_regularizer = regularizers.l2( weight_decay ) )( dropout_layer )

        # if self.final_layer_type == "arcface":
        #     output_layer = ArcFace( number_of_outputs, margin = self.margin, scale = self.scale, activation = "multi_label", regularizer = regularizers.l2( weight_decay ) )( [dense_layer, cosface_input] )
        # elif self.final_layer_type == "sphereface":
        #     output_layer = SphereFace( number_of_outputs, margin = self.margin, scale = self.scale, activation = "multi_label", regularizer = regularizers.l2( weight_decay ) )( [dense_layer, cosface_input] )
        # elif self.final_layer_type == "cosface":
        #     output_layer = CosFace( number_of_outputs, margin = self.margin, scale = self.scale, activation = "multi_label", regularizer = regularizers.l2( weight_decay ) )( [dense_layer, cosface_input] )
        # else:
        #     output_layer = Dense( units = number_of_outputs, activation = self.activation_function, name = "Output_Layer" )( dense_layer )

        self.model = Model( inputs = [token_input_ids, attention_mask, token_type_ids], outputs = output_layer, name = self.network_model + "_model" )
        ...
        # Print Model Summary
        self.Print_Log( "CLBERTModelDistributed::Build_Model() - =========================================================" )
        self.Print_Log( "CLBERTModelDistributed::Build_Model() - =                     Model Summary                     =" )
        self.Print_Log( "CLBERTModelDistributed::Build_Model() - =========================================================" )

        self.model.summary( print_fn = lambda x: self.Print_Log( "CLBERTModelDistributed::Build_Model() - " + str( x ) ) )      # Capture Model.Summary()'s Print Output As A Function And Store In Variable 'x'
...
if __name__ == '__main__':
    ...
    print( "     from models import CLBERTModelDistributed\n" )
    print( "     model = CLBERTModelDistributed( print_debug_log = True, per_epoch_saving = False, use_csr_format = True )" )
