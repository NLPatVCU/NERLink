...
from NERLink.Layers                import Embedding_Extraction_Layer, ArcFace, CosFace, SphereFace
...
class ModelDataGenerator( DataGenerator ):
    ...
    def __getitem__( self, index ):
        ...
        token_input_ids, attention_masks, token_type_ids, entry_term_masks = self.X
        ...
        X_entry_term_masks = entry_term_masks[batch_indices,:]
        Y_input            = self.Y[batch_indices,:].todense()
        Y_output           = Y_input
        ...
        return [X_token_input_ids, X_attention_mask, X_token_type_ids, X_entry_term_masks, Y_input], Y_output, sample_weights
...
class CLBERTModel( BaseModel ):
    def __init__( self, print_debug_log = False, write_log_to_file = False, margin = 30.0, optimizer = 'adam', activation_function = 'softmax',
        ...
        self.version       = 0.09
        self.network_model = "concept_linking_bert"
        self.bert_config   = None

        if self.Get_Is_Using_TF2() == False:
            self.Print_Log( "CLBERTModel::__init__() - Error: BERT Model Only Supports Tensoflow >= 2.x", force_print = True )
            exit()
    ...
    def Batch_Generator( self, X, Y, batch_size, steps_per_batch, shuffle ):
        token_input_ids, attention_masks, token_type_ids, entry_term_mask = X
        ...
        while True:
            ...
            X_entry_term_mask = entry_term_mask[batch_index,:]
            Y_input           = Y[batch_index,:].todense()
            Y_output          = Y_input
            counter           += 1

            yield [X_token_input_ids, X_attention_mask, X_token_type_ids, X_entry_term_mask, Y_input], Y_output
    ...
    def Fit( self, inputs = None, outputs = None, val_inputs = None, val_outputs = None, epochs = None, batch_size = None,
        ...
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModel::Fit() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return False

        if outputs is None or not outputs or len( outputs ) == 0:
            self.Print_Log( "CLBERTModel::Fit() - Error: Expected Output Data Length Mismatch", force_print = True )
            return False
        ...
        self.Print_Log( "CLBERTModel::Fit() - Model Training Settings" )
        ...
        self.Print_Log( "CLBERTModel::Fit() - Executing Model Training", force_print = True )
        ...
        with tf.device( self.device_name ):
            ...
            if self.Get_Is_Using_TF1() and self.use_csr_format:
                ...
                self.model_history = self.model.fit( [input_ids, attention_masks, token_type_ids, entry_term_masks, outputs], outputs, validation_data = validation_data,
                                                      shuffle = self.shuffle, batch_size = self.batch_size, epochs = self.epochs, verbose = self.verbose,
        ...
        if self.verbose == False:
            ...
            self.Print_Log( "CLBERTModel::Final Training Metric(s) At Epoch: " + str( final_epoch ), force_print = True )

            # Iterate Through Available Metrics And Print Their Formatted Values
            for metric in history.keys():
                self.Print_Log( "CLBERTModel::  - " + str( metric.capitalize() ) + ":\t{:.4f}" . format( history[metric][-1] ), force_print = True )

        self.Print_Log( "CLBERTModel::Fit() - Finished Model Training", force_print = True )
        self.Print_Log( "CLBERTModel::Fit() - Complete" )
        return True
    ...
    def Predict( self, inputs, verbose = 0 ):
        # Check(s)
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModel::Predict() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return None

        self.Print_Log( "CLBERTModel::Predict() - Predicting Using Inputs: " + str( inputs ) )
        ...
        with tf.device( self.device_name ):
            ...
            cosface_inputs = np.zeros( ( input_ids.shape[0], self.Get_Number_Of_Outputs() ), dtype = np.int32 )
            ...
            return self.model.predict( [input_ids, attention_masks, token_type_ids, entry_term_masks, cosface_inputs],
    ...
    def Evaluate( self, inputs, outputs, verbose, warmup_model = True ):
        # Check(s)
        if inputs is None or ( isinstance( inputs, tuple ) and len( inputs ) < 4 ):
            self.Print_Log( "CLBERTModel::Evaluate() - Error: Expected Input Tuple Of 4 Elements", force_print = True )
            return -1, -1, -1, -1, -1

        if outputs is None or len( outputs ) == 0:
            self.Print_Log( "CLBERTModel::Evaluate() - Error: Expected Output Data Length Mismatch", force_print = True )
            return -1, -1, -1, -1, -1

        self.Print_Log( "CLBERTModel::Evaluate() - Executing Model Evaluation" )
        ...
        with tf.device( self.device_name ):
            ...
            loss, accuracy, precision, recall, f1_score = self.model.evaluate( [input_ids, attention_masks, token_type_ids, entry_term_masks, outputs], outputs,
                                                                                batch_size = self.batch_size, verbose = verbose )

            self.Print_Log( "CLBERTModel::Evaluate() - Complete" )
    ...
    def Build_Model( self, max_sequence_length = 0, number_of_unique_terms = 0, embedding_dimension_size = 0,
        ...
        self.Print_Log( "CLBERTModel::Build_Model() - Model Settings" )
        ...
        if self.final_layer_type not in final_layer_type_list:
            self.Print_Log( "CLBERTModel::Build_Model() - Error: Invalid Final Layer Type", force_print = True )
        ...
        cosface_input       = Input( shape = ( number_of_outputs,   ), name = "CosFace_Input",         dtype = tf.int32 )
        ...
        # ========================================================================================================= #
        # NOTE: Found Removing These Layers In Favor Of A Single Dense Output Layer Improves Performance.           #
        ...
        output_layer = Dense( units = number_of_outputs, input_dim = embedding_dimension_size, activation = self.activation_function, name = "Output_Layer" )( sub_word_embeddings )

        self.model = Model( inputs = [token_input_ids, attention_mask, token_type_ids, entry_term_mask, cosface_input], outputs = output_layer, name = self.network_model + "_model" )
        ...
        # Print Model Summary
        self.Print_Log( "CLBERTModel::Build_Model() - =========================================================" )
        self.Print_Log( "CLBERTModel::Build_Model() - =                     Model Summary                     =" )
        self.Print_Log( "CLBERTModel::Build_Model() - =========================================================" )

        self.model.summary( print_fn = lambda x: self.Print_Log( "CLBERTModel::Build_Model() - " + str( x ) ) )      # Capture Model.Summary()'s Print Output As A Function And Store In Variable 'x'
...
if __name__ == '__main__':
    ...
    print( "     from models import CLBERTModel\n" )
    print( "     model = CLBERTModel( print_debug_log = True, per_epoch_saving = False, use_csr_format = True )" )
